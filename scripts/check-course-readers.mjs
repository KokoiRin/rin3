...
export async function validateCourseReaders(courseSlugs = learningCourseSlugs) {
  ...
  for (const courseSlug of courseSlugs) {
    ...
    const readerCss = readerCssByCourse.get(courseSlug) ?? path.join(courseRoot, "reader.css");
    const htmlFiles = await listHtmlFiles(courseRoot);
    const chapterFiles = htmlFiles.filter((file) => file !== courseIndex);

    if (!await exists(readerCss)) errors.push(`${courseSlug}: missing configured reader.css`);
    ...
    for (const filePath of htmlFiles) {
      ...
      if (!resolvedStylesheets.includes(readerCss)) {
        errors.push(`${relativePath}: must reference the configured reader.css`);
      }
