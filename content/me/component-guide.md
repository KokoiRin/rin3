...
内容角色描述一段话是核心判断还是详细解释；投影规则决定它进入文章、Slides 或未来的其他视图；presentation 再决定 Slides 使用卡片、流程或普通文字展示。当前普通内容默认是 core，detail 只进入文章。

这段说明故意作为 detail：你能在文章中读到它，但对应的 Slides 首页仍然只保留上面的三个核心要点。
:::
:::
...
- **Content**：`content/me/*.md` 保存共享元数据、普通 Markdown 正文、显式分页边界和布局类型，是内容的唯一来源。
- **Compiler**：`RinDocument` 先解析成中立模型，再分别生成文章 HTML 与 `SlideDeckData`；非法结构会在构建时失败。
- **Player**：`slide-deck.tsx` 负责章节跳转、键盘、触摸、进度、深链接、全屏和响应式展示，不理解源文档语法。
:::
...
- **Identity**：`title / lang / date` 决定分享标题、内容语言、发布日期和两个视图共同显示的身份。
- **Discovery**：`topic / tags / order` 决定文章在所属分区中的摘要、标签和排序；双视图内容只出现一次。
- **Presentation**：`series / coverImage` 只补充 Slides 所需的系列名和封面图，不复制正文语义。
- **Structure**：`chapters / slide directives` 定义稳定导航、页面边界和布局；分页始终由作者控制。
:::
...
| 并列说明 | cards / contract | 使用“标题：说明”无序列表组织单元 |
...
| `- 标题：说明` 列表项 | `cards.items` / `contract.items` | 普通说明列表 |
...
| `mermaid` fenced code | `prose.html` | Mermaid 图表 |
| `![alt](path)` | `prose.html` | 响应式图片 |
...
:::slide {"kind":"prose","chapter":"rich-content","eyebrow":"RICH CONTENT / MERMAID"}
## Mermaid 围栏同时进入文章和 prose Slides

作者只保存图表定义。页面按需加载 Mermaid，并把同一段结构渲染成 SVG；不包含图表的页面不会下载 Mermaid 运行时。

```mermaid
flowchart LR
  Source["RIN Markdown"] --> Article["Article view"]
  Source --> Slides["Prose Slides"]
  Article --> Reader["Linear reading"]
  Slides --> Presenter["Progressive presentation"]
```
:::

:::slide {"kind":"prose","chapter":"rich-content","eyebrow":"RICH CONTENT / IMAGE"}
## 标准 Markdown 图片会自动适配部署路径

图片放在 `public/` 下，正文使用以 `/` 开头的站内路径。构建到 GitHub Pages 时，宿主会自动补上 `/rin3`，文章和 Slides 不需要维护两份地址。

![莲花与计算机科学入口图](/entrance/computer-lotus.webp)
:::

:::slide {"kind":"prose","chapter":"rich-content","eyebrow":"RICH CONTENT / LISTS"}
## 普通列表提供数字标号和圆点 item
...
- 每一项前面使用清晰、克制的黑色圆点。
- 文本仍然保留标准 Markdown 列表语义。
- 文章和 prose Slides 使用同一套内容约定。
:::
...
- **Markdown math**：源文件保留可移植的 LaTeX 本体，不保存生成后的 HTML。
- **KaTeX HTML**：两个视图分别渲染同一表达式，浏览器不在运行时解析公式。
:::
